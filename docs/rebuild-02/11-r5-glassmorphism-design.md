...
- **Last updated:** 2026-08-08 (R5-7 QA complete — all targets green; **R5-8a pre-deploy polish cycle (DL-063) in progress** — T1…T5 one-at-a-time on `evolve-design`, awaiting owner's R5-8 deploy go/no-go after the polish cycle)
...
| R5-5 | **Case-study + inner pages** | Sidebar glass, case-cover frame, page-hero glass band, mobile drawer glass | All 3 studies; both themes | ✅ **Complete** (DL-060; evidence `21-r5-5-inner-pages-evidence.md`) |
| R5-6 | **Architecture cleanup** | `SectionHeader`, `GlassPanel`, split style blocks, delete dead code, unify durations, extend `check-contrast` for glass tokens | `npm run build` passes; diff is refactor-only (no visual change) | ✅ **Complete** (DL-061; evidence `22-r5-6-architecture-evidence.md`) |
| R5-7 | **Full QA report** | Re-run the R3-F matrix **with** glass: both themes, back-of-glass, reduced motion/transparency, no-JS, Lighthouse, axe, overflow, links, contrast-incl-glass; evidence doc `docs/rebuild-02/12-r5-qa-evidence.md` | All targets green; rollback recorded | ✅ Complete 2026-08-08 (DL-062): all M1–M12 green — 19/19 + 30/30 AA, Lighthouse 100×24/24, axe 0, overflow 0, motion/transparency fallbacks proven → owner GO → deploy |
...
| 2026-08-08 | **R5-5 case-study + inner pages complete** (DL-060): case-page hero → glass band over a masked `--color-glow` field (`case-hero.band-host` + `.section-band` aria-hidden + `.case-hero-inner.glass-band`); case sidebar cards → glass (tint + border + radius, no blur); case-cover → glass frame; mobile nav drawer → navbar glass recipe (blur included) with `@supports` + reduced-transparency fallbacks; body prose stays flat (§10). Contrast script extended with the **glow-peak centre scenario** (D4, case-hero band sits at peak not card-zone half): on-glass checks 24 → **30**; centre failures resolved by minimal token raises — dark `ink-muted` `#888→#8d8d8d` (4.63:1), dark `ink-dim-glass` `#848→#8b8b8b` (4.51:1), light `ink-dim-glass` `#6c6c6c→#6a6a6a` (4.56:1). Evidence: `docs/rebuild-02/21-r5-5-inner-pages-evidence.md`. Verified: build 6 pages, `check:contrast` **19/19**, on-glass **30/30** AA, Lighthouse **100/100/100/100** on a case page (mob+desk, TBT 0, CLS 0), axe 0 (dark+light), overflow 0 (320–1440), reduced-motion untouched. Gate open: R5-6. |
| 2026-08-08 | **R5-6 architecture cleanup complete** (DL-061): created `SectionHeader.astro` + `GlassPanel.astro` (shared recipes; `GlassPanel` first wired into the home hero plate); split `index.astro`'s 7 sections into `src/sections/*` (each with scoped styles, data via props; `index` is now a thin composition with no `<style>` block); promoted shared scaffolding (`.section-block`/`.section-alt`/`.band-host`/`.section-band`) to `global.css`, deleting the duplicated scoped copies from `index.astro` and `[slug].astro` (slug keeps its `case-hero` mask override); deleted dead tokens (elevation/paper/hairline/border-focus/accent-ink/gradient aliases, `.glow-pulse`, `.delay-600`, `.animate-fade-in` — all zero-verified); unified duration literals to seeds (3 swaps); fixed `check-contrast.mjs` drift to the live R5-5 hexes. Evidence: `docs/rebuild-02/22-r5-6-architecture-evidence.md`. Verified: build 6 pages clean, `check:contrast` **19/19**, on-glass **30/30** AA, Lighthouse 100s (mob+desk, TBT 0), axe 0 (dark+light, settled render), overflow 0 (320–1440), **rendered-output diff vs `HEAD` = zero visual change**. Gate open: R5-7. |
| 2026-08-08 | **R5-7 full QA report complete** (DL-062): re-ran the R3-F matrix with the R5 glass layer live and extended it with the glass-specific scenarios. All M1–M12 targets green on the local release artifact — build 6 pages, `check:contrast` **19/19**, on-glass **30/30** AA, **Lighthouse 100/100/100/100 on 24/24** cells (6 routes × mob/desk × light/dark, TBT 0, CLS 0), axe **0** (settled) all routes × themes, overflow **0 (20/20)**, reduced-motion static, reduced-transparency → solid no-blur, back-of-glass confirmed, no-JS content intact, links healthy (LinkedIn 999 authwall benign), console 0 errors, metadata/SEO retained. Evidence: `docs/rebuild-02/12-r5-qa-evidence.md`. **Owner gate: R5-8 deploy go/no-go.** |
| 2026-08-08 | **R5-8a pre-deploy polish cycle planned (DL-063):** owner brainstorm → five pre-R5-8 enhancement themes on `evolve-design`, implemented **one at a time** (each owner-gated), then R5-8 deploy. Plan: `docs/rebuild-02/24-r5-8a-polish-interaction-plan.md`. T1 remove hero glass card; T2 portrait favicon + `<title>`/manifest; T3 testimonial premium redesign; T4 cursor-centered background spotlight replacing static bands + neon cursor glow; T5 decoration-only `@chenglou/pretext` marquee (**owner-approved S-12 override**, CSS fallback). Shared done-when: build 6 pages · contrast 19/19 + on-glass 30/30 · Lighthouse 100s · axe 0 · overflow 0 · reduced-motion/transparency · no-JS · JS-budget · zero claim changes. **Start: T1.** |
