...
| CL-21 | Better DEV — code-verified engineering decisions (2026-08-22): AI SDK v5 image-strip workaround; vision-model retirement → OCR fallback; SSE error-filter short-circuit; stream cancellation on client disconnect; classifier routing ladder (<15-char heuristic, MD5 cache 5-min TTL/1000-entry FIFO, AI 5s timeout, fail-safe Fast); token budget (32k document/64k total, ≈4 chars/token) + 5-image cap + `MAX_TOOL_ITERATIONS=5`; provider-parameterized S3 storage; composite indexes + `DISTINCT ON` sidebar query | SRC-09 repos, code inspection (2026-08-22) | High | VERIFIED | Decisions + code-level guardrails only; no metrics; "production-shaped practices" wording per owner gate 2026-08-22 (DL-082) — never "production-grade" |
| CL-22 | Better DEV — honest quality posture (2026-08-22): Jest unit suites exist exactly for auth, mode resolution, message utils, storage, chat DTOs; CI on both repos = build verification; streaming/tool paths and frontend untested | SRC-09 repos, spec-file inspection (2026-08-22) | High | VERIFIED | State test scope precisely; no generic "tested" claims |
| CL-23 | Better DEV — object storage is **deployed on Supabase Object Storage** (S3 client, path-style, `better-dev-attachments` bucket); Cloudflare R2 was the original plan and was dropped for cost, remaining only a documented S3-compatibility target (owner 2026-08-22; `.env.example`/`storage.service.ts` document R2/Supabase/Spaces/S3 as options) | Owner directive (2026-08-22); SRC-09 repo inspection | High | VERIFIED | Public copy presents the technology generically: "S3-compatible object storage" / "object storage" — Supabase is the hosting service and must not be named on the site (owner 2026-08-22, DL-084); never claim zero-egress or R2-in-use |
