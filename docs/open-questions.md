...
| OQ-06 | Which LinkedIn URL is canonical: `in/kashif-rezwi` (portfolio + GitHub profile) or `in/kashif-rezwi-149372216` (resume + PROFILE.md)? Neither was verifiable without login. | Conflict C-01 (evidence ledger) | Kashif (owner) | Provide the canonical URL — or a profile paste/export, which would also resolve OQ-01. | Open |
| OQ-07 | Confirm the GitHub-profile aspiration ("AI labs, model training, research workflows") stays out of portfolio positioning, per the approved claim boundaries. | Conflict C-02 (evidence ledger) | Kashif (owner) | Explicit confirmation. | Open |
| OQ-08 | Which employer-product details (Swipe Pages / Swipe One) may be shown publicly in an experience section — feature names, descriptions, screenshots? | Featured-project research (shared gaps); ledger CL-03…CL-11 | Kashif (owner) | Item-by-item disclosure approval. | Open |
| OQ-09 | How should bootcamp group projects be handled (Myntra/Nordstrom/NearBuy/Trendsy/codeair): removed entirely, archived, or kept with explicit learning labels? Includes the teammate-hosted repo risk (C-07). | github-project-triage.md | Kashif (owner) | Decision. | Open |
| OQ-10 | May `slooze-ai-agent-challenge` (a job-application challenge) remain public and/or be shown anywhere? | github-project-triage.md | Kashif (owner) | Permission decision. | Open |
| OQ-11 | Register correction: SRC-04b (`data/sources.csv`) is job-source tracking, not a LinkedIn substitute. Editing `docs/source-register.md` was outside Phase 0B's allowed edit list — approve amending it. | Conflict C-11 (evidence ledger) | Kashif (owner) | Approval to amend `docs/source-register.md`. | Open |