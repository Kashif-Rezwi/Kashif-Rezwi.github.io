...
## DL-098 — LingoAgent case study: full upgrade to 7-section engineering structure (approved plan implementation)

- **Date:** 2026-09-13
- **Context:** Owner-approved implementation plan ("Lingo Agent — Portfolio Case Study Analysis & Implementation Plan"). More-tier case study (order 6) upgraded from 4 thin sections to the full 7-section engineering structure. Independent pre-implementation cross-check performed against the live `main` README (`raw.githubusercontent.com/Kashif-Rezwi/lingo-agent/main/README.md`, fetched 2026-09-13) — not trusting the plan's transcriptions.
- **Implementation:** `src/content/work/lingo-agent.md` rewritten —
  1. Frontmatter `summary` → full-stack pipeline framing (7 deterministic tools, E2B sandbox, Babel AST, SSE); `tech` expanded 7 → 14 (added NestJS, Vercel AI SDK, Groq, PostgreSQL, Prisma, RxJS, NextAuth.js — all confirmed in the README Tech Stack table). Narrative footnote removed per plan.
  2. Body: 7 sections — Overview (full-stack map + live links), The Problem (orchestration gap), Architecture (README diagram verbatim), How It Works (7-tool table), Engineering Decisions (6: LLM-as-planner, E2B isolation, Babel loaded from target repo's node_modules, regex fallback, RxJS ReplaySubject per job, MCP for dynamic setup), Known Limitations (4 deliberate scope decisions), Outcomes (3 live links + estimate-vs-benchmark disclaimer). No marketing language; no new claims beyond README/source-verified facts.
- **Plan deviations (documented, evidence-driven):**
  1. **Diagram:** plan's pasted ASCII diagram is a paraphrase (wrong arrow style, omits `/api` prefix + Swagger lines, merged column labels). Plan §3/§9 itself instructs "reproduce the README diagram verbatim — no fabrication"; used the actual README System Architecture box-drawing diagram, verified byte-level against `main`.
  2. **Tool name:** plan's step-6 `commit_push` corrected to README-verified `commit_and_push`.
  3. **Per-step estimate:** plan draft's "Each step individually is 10–15 minutes" has no source support (README states only the aggregate "3–5 hours manual" + "~3 minutes automated" in the Orchestration Gap diagram, both cited as author-stated estimates) — softened to "Each step is minutes of routine work" to honor AGENTS.md §3 (no invented metrics).
  4. **CSS gap (plan §9 assumption false):** `.markdown` styles had no `pre` or `table` rules — no case study had ever used them. Added minimal token-based rules to the existing `.markdown` block in `global.css` (mono fenced block on accent-soft with `overflow-x: auto`; bordered table; ≤640px tables become scroll-in-place boxes). No component or layout changes; no new tokens (contrast scripts unaffected).
  5. README's "one tool schema at a time" enforcement (README: `toolChoice: 'required'`) added to How It Works (README-verified, strengthens the planner/executor distinction).
  6. **More row terminology (owner's MORE PROJECT RULE):** plan §12 listed `Work.astro` out of scope, but its LingoAgent row still read "Autonomous i18n agent…" — phrasing the owner ruled out in DL-004 and now inconsistent with the upgraded case study. One-line description text updated to mirror the new summary (full-stack AI agent pipeline / 7 deterministic tools / E2B sandbox / Babel AST / auto PR with preview deploy); links and structure untouched.
- **Verification:** `npm run build` clean — 9 pages; `/work/lingo-agent/` renders Overview → Problem → Architecture (pre with diagram) → How It Works (table) → Engineering Decisions (6 bold leads) → Known Limitations (4 bullets) → Outcomes (3 links + disclaimer); More row links [Read case study, Repo, Live] unchanged in `Work.astro`; cover `lingo-agent.png` confirmed present in `src/assets/work/` (plan Risk 1 cleared); no other case study, template, or section file touched. Contrast checks run post-build (results in project-status).
- **Files changed:** `src/content/work/lingo-agent.md` · `src/sections/Work.astro` (More-row description text only) · `src/styles/global.css` · `docs/decision-log.md` · `docs/project-status.md`.
- **Status:** Working-tree changes ready for owner review. Not committed; not pushed; deployment remains owner-gated (AGENTS.md §7).

## DL-099 — LingoAgent case study: final cross-check & review fixes

- **Date:** 2026-09-13
- **Context:** Owner-directed final quality review (independent re-fetch of the live repo — full tree via GitHub API, README, `server/package.json`, `client/package.json`, `schema.prisma`, `agent.service.ts`, `install-translate.tool.ts`, `commit-push.tool.ts`, `detect-framework.tool.ts`, `analyze-repo.tool.ts`, `mcp.service.ts`, e2e stub — not trusting DL-098 or the plan).
- **Re-verification findings (all against live `main`, last push 2026-09-12):**
  1. **Every factual claim CORRECT:** Next.js 14.2.14 + NextAuth 4.24.13 (client manifest); @nestjs/core ^11.0.1, prisma ^7.4.1, ai ^6.0.96, @ai-sdk/groq ^3.0.24, e2b ^2.12.1, rxjs ^7.8.1, @octokit/rest ^22.0.1, lingo.dev ^0.131.7, @modelcontextprotocol/sdk ^1.26.0 (server manifest); `toolChoice: 'required'` (agent.service.ts:211); per-job `ReplaySubject<SseEvent>` + cancel → abort + immediate `sandbox.kill()` (agent.service.ts:32-97); Babel dynamically imported from the target repo's `node_modules` with grep/regex fallback (install-translate.tool.ts:80-96, 255-309); `commit_and_push` tool name + `lingo/add-multilingual-<timestamp>` branch (commit-push.tool.ts); throw-on-non-App-Router (detect-framework) and throw-on-conflicting-i18n-library (analyze-repo) stop behaviors verified verbatim in tool docstrings/code; JobStatus enum pending/running/completed/failed/cancelled (schema.prisma); no `.github` directory (no CI/CD — CORRECT); only the scaffold `app.e2e-spec.ts` (CORRECT).
  2. **No incorrect, unsupported, or outdated claims found on the page or More row.** Estimate framing (3–5 hours manual / ~3 minutes automated, author-stated) matches the README Orchestration Gap diagram exactly.
- **Fixes applied (owner directive: no ugly separators/dividers):**
  1. Removed all 6 body `---` horizontal rules from `lingo-agent.md` — every other case study has zero body dividers and `.markdown` has no `hr` styling, so they rendered as unstyled browser-default `<hr>`s; frontmatter delimiters (the only 2 `---` lines) preserved; double blank lines left by removals collapsed.
  2. Writing tightening in The Problem: closing sentence de-duplicated ("LingoAgent addresses this as an orchestration problem…" → "LingoAgent automates that entire sequence as a single agent pipeline.").
...
- **Date:** 2026-09-16
- **Context:** The owner no longer wants to re-upload the resume PDF to this repo on every update. Previously (OQ-05, DL-007) the resume was served as a same-origin PDF at `/resume/Kashif-Rezwi-Resume.pdf` and the Google Drive fork was dropped (resolved C-08). The owner now explicitly reverses that: the Drive file `https://drive.google.com/file/d/13lXkje4gkeewEEel2amsvXtRmjPLmUEG/view` becomes the single source of truth, with the site's existing "opens in a new tab AND downloads at the same time" behavior preserved.
- **Decision:** Replace the local PDF with two Drive URLs derived from one file-ID constant in `src/lib/paths.ts` (`resumeViewUrl` = Drive preview viewer; `resumeDownloadUrl` = `uc?export=download` export endpoint). The `Base.astro` click interceptor now matches `a[data-resume-link]` instead of `.pdf` hrefs: it opens `href` (viewer) in a new tab and synthetic-clicks `data-resume-download` (export URL). No `download` attribute is used — Drive responds `Content-Disposition: attachment; filename="Kashif-Rezwi-Resume.pdf"` (verified 2026-09-16 via response headers), and cross-origin `download` attributes are ignored by browsers anyway. All 5 consumers updated (Header desktop+mobile nav, Hero CTA, About CTA, Footer icon link, `/resume` hub buttons). Local `public/resume/Kashif-Rezwi-Resume.pdf` deleted. The `/resume` page remains as the on-site hub.
- **Operational rules (owner-facing):** (1) update the resume by uploading a NEW VERSION to the SAME Drive file (Manage versions) — a new file means a new ID and broken links; (2) keep sharing at "Anyone with the link"; (3) keep the Drive file name `Kashif-Rezwi-Resume.pdf` — it is the filename visitors download.
- **Known trade-off (accepted by owner):** the "open" tab shows the Google Drive preview viewer instead of the browser's native PDF viewer — Drive serves no raw-inline-PDF URL for shared files (`export=view` also returns `Content-Disposition: attachment`; verified 2026-09-16).
- **Governing-doc updates:** `docs/decision-log.md` (this entry) · `docs/project-status.md` (Phase 0 gate note amendment + working-tree note) · `docs/source-register.md` (SRC-17 added; privacy flag updated).
...
- **Decision:** Added a new first entry (newest first) to the `exp-edu-list` in `src/sections/Experience.astro`: **Bosscoder Academy** — *Advanced AI & ML Engineering Program · Jun 2026 – present*. Classification: **Learning**, label **VERIFIED** (owner statement + official page). Module names are not listed on the site, matching the existing minimal entry style (Masai, Amity). **Owner revisions same day:** removed the "(expected Mar 2027)" suffix (owner prefers no projected end date), and corrected the Amity BCA start to **Apr 2025** — *BCA (Bachelor of Computer Applications) · Apr 2025 – present* — superseding the evidence ledger's Jul 2025 (owner correction is source-hierarchy tier 1; ledger §1 updated).
