...
**Phase 1.4 — Production launch: IN PROGRESS** (Phase 1.3 gate approved 2026-08-05, DL-013). This is the irreversible launch phase: flip GitHub Pages source from `gh-pages` branch to the `deploy.yml` workflow, verify production renders the Astro site (HTTP 200, zero JS, `noindex` removed), archive the preview repo, close all Phase 1 docs. Production URL must switch from the CRA bundle to the new Astro site — this is the only write to the production path.
...
- **Phase 1.4 — Production launch** (DL-013 approved): flip GitHub Pages source from `branch: gh-pages` to the `deploy.yml` workflow; verify production renders Astro (HTTP 200, zero JS confirmed, `noindex` removed); verify preview repo archived; confirm all docs closed. Production URL switches from CRA bundle to new Astro site — this is the only irreversible write to production, and it is authorized by the owner at this gate.
- **Phase 1.3 — COMPLETE, gate approved** (2026-08-05, DL-013). QA report (`docs/engineering/qa-report.md`) finalized: Lighthouse 100/100/100/100 (home + case-study pages); zero client JS; LCP 1.4 s / 1.3 s; CLS 0; TBT 0 ms; WCAG 2.2 AA (0 axe failures); `prefers-reduced-motion`; unique per-page titles/descriptions/OG; sitemap + robots; dedicated OG card live; 404 page; manifest + favicon; full link check complete. Production verified untouched (still serving CRA; Pages source still `branch: gh-pages`).